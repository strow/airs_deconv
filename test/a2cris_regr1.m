%
% a2cris_regr1 - AIRS to CrIS Tb regression corrrection
%
...
iind = ~idep;
acdep = acbt(:, idep);  % AIRS CrIS dependent set
acind = acbt(:, iind);  % AIRS CrIS independent set
tcdep = tcbt(:, idep);  % true CrIS dependent set
tcind = tcbt(:, iind);  % true CrIS independent set
[~, ndep] = size(acdep);
[~, nind] = size(acind);
